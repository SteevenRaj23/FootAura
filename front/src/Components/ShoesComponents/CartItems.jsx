  ...
  return (
    <div className="flex justify-between border border-gray-200 rounded-2xl h-30 w-[800px] mt-10 ">
      <div className="flex gap-2">
        <div>
          <img src={Items.productId.images[0]} className="ml-4 h-25 w-25 p-1 object-contain"></img>
        </div>
      ...
      <div className="flex flex-col items-end justify-between p-6">
        <FaRegTrashAlt className="text-red-500" onClick={()=>removeItem(Items.productId._id,Items.size,Items.quantity)}/>
        <h1 className="text-2xl font-bold">${Items.productId.price}</h1>
      </div>
    </div>
  );
}
